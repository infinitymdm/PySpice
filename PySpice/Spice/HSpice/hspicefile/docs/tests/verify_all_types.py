  ...
  with open(filename, "rb") as f:
    ...
    for t_raw in tables_raw:
      ...
      if meta["num_sweeps"] > 0:
        sweep_val = struct.unpack(fmt + sweep_val_fmt, t_raw[:sweep_val_size])[0]
      ...
      for _ in range(num_rows):
        ...
        for j, name in enumerate(meta["var_names"]):
          ...
          if is_complex and j > 0:
            if vsz == 16:
              re, im = struct.unpack(fmt + "dd", chunk)
            else:
              re, im = struct.unpack(fmt + "ff", chunk)
            table[name].append(complex(re, im))
          else:
...
def make_tests():
  ...
  def dc_sweep_temp_assertions(tables, meta):
    ...
    if scale not in t:
      ...
      return errs
    if len(t[scale]) != 5:
      errs.append(f"Expected 5 rows (temp -40 to 120 step 40), got {len(t[scale])}")
  ...
  def tran_probe_and_sweep_assertions(tables, meta):
    ...
    for i, t in enumerate(tables):
      for partial in expected_probe_partials:
        ...
        if not found:
          errs.append(f"Table {i}: expected probe containing '{partial}' not found. Keys={list(t.keys())}")
    return errs
  ...
  def dc_nested_sweep_assertions(tables, meta):
    ...
    for i, t in enumerate(tables):
      if scale not in t:
        ...
        continue
      if len(t[scale]) != 5:
        errs.append(f"Table {i}: expected 5 inner points (v1 1..5), got {len(t[scale])}")
    return errs
  ...
  def dc_meas_assertions(tables, meta):
    ...
    if "v_out_max" not in meas:
      errs.append(f"Expected measurement 'v_out_max' not found (keys={list(meas.keys())})")
    else:
      val = meas["v_out_max"]
  ...
  def ac_meas_assertions(tables, meta):
    ...
    if "v_out_at_1k" not in meas:
      errs.append(f"Expected measurement 'v_out_at_1k' not found (keys={list(meas.keys())})")
    else:
  ...
  def dc_meas_sweep_assertions(tables, meta):
    ...
    if "v_out_max" not in meas:
      ...
      if not isinstance(vals, list) or len(vals) != len(expected) or not all(_close(x, y) for x, y in zip(vals, expected)):
...
def main():
  ...
  with open(log_path, "w") as lf:
    ...
    for bin_file, ascii_file, case_name, assertion_fn in tests:
      ...
      log(f"    Version: {meta['version']}  Vectors: {meta['num_vectors']}  "
      ...
      if ascii_file is not None and ascii_file != bin_file:
        ...
        try:
          ...
          if ascii_tables is None:
            errs.append(f"ASCII golden file not found or failed to parse ({ascii_path})")
          elif len(tables) != len(ascii_tables):
            errs.append(f"Number of tables mismatch: binary has {len(tables)}, ASCII has {len(ascii_tables)}")
          else:
            for t_idx, (bin_table, ascii_table) in enumerate(zip(tables, ascii_tables)):
              bin_keys = [k for k in bin_table.keys() if k != "__sweep_val__"]
              ascii_keys = [k for k in ascii_table.keys() if k != "__sweep_val__"]
              if len(bin_keys) != len(ascii_keys):
                errs.append(f"Table {t_idx} variable count mismatch: binary has {len(bin_keys)}, ASCII has {len(ascii_keys)}")
                continue
              for key in bin_keys:
                bin_vals = bin_table[key]
                ascii_vals = ascii_table[key]
                if len(bin_vals) != len(ascii_vals):
                  errs.append(f"Table {t_idx} variable '{key}' length mismatch: binary has {len(bin_vals)}, ASCII has {len(ascii_vals)}")
                  continue
                for val_idx, (b_v, a_v) in enumerate(zip(bin_vals, ascii_vals)):
                  if isinstance(b_v, complex):
                    if not _close(b_v.real, a_v.real, rtol=1e-3) or not _close(b_v.imag, a_v.imag, rtol=1e-3):
                      errs.append(f"Table {t_idx} variable '{key}' row {val_idx} value mismatch: binary={b_v}, ASCII={a_v}")
                      break
                  else:
              ...
              if "__sweep_val__" in bin_table:
                if "__sweep_val__" not in ascii_table:
                  errs.append(f"Table {t_idx} sweep value missing in ASCII table")
                elif not _close(bin_table["__sweep_val__"], ascii_table["__sweep_val__"], rtol=1e-3):
                  errs.append(f"Table {t_idx} sweep value mismatch: binary={bin_table['__sweep_val__']}, ASCII={ascii_table['__sweep_val__']}")
    ...
    log("\n" + "=" * 70)
    log(f"Summary: {passed} passed, {failed} failed, {skipped} skipped"
