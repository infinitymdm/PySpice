...
def _unpack(result):
  ...
  assert isinstance(sweeps, tuple) and len(sweeps) == 3, \
...
def make_tests():
  ...
  def dc_basic(title, date, scale_name, sweep_name, sweep_vals, data_list):
    ...
    sn = scale_name.lower()
    if sn not in t:
      errs.append(f"Scale key '{sn}' not in dict (keys={list(t.keys())})")
      return errs
    arr = t[sn]
  ...
  def dc_sweep_temp(title, date, scale_name, sweep_name, sweep_vals, data_list):
    ...
    sn = scale_name.lower()
    if sn not in t:
      errs.append(f"Scale key '{sn}' not found (keys={list(t.keys())})")
      return errs
    arr = t[sn]
    if len(arr) != 5:
      errs.append(f"Expected 5 rows, got {len(arr)}")
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"dc_sweep_temp_{version}.sw0", f"dc_sweep_temp_{version}", dc_sweep_temp))
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"dc_sweep_param_{version}.sw0", f"dc_sweep_param_{version}", dc_sweep_param))
  ...
  def dc_monte(title, date, scale_name, sweep_name, sweep_vals, data_list):
    ...
    for i, t in enumerate(data_list):
      if sn not in t:
        errs.append(f"Table {i}: scale '{sn}' missing (keys={list(t.keys())})")
        continue
      arr = t[sn]
      if not isinstance(arr, np.ndarray):
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"dc_nested_sweep_{version}.sw0", f"dc_nested_sweep_{version}", dc_nested_sweep))
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"dc_probes_only_{version}.sw0", f"dc_probes_only_{version}", dc_probes_only))
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"dc_probe_and_sweep_{version}.sw0", f"dc_probe_and_sweep_{version}", dc_probe_and_sweep))
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"ac_sweep_param_{version}.ac0", f"ac_sweep_param_{version}", ac_sweep_param))
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"ac_sweep_temp_{version}.ac0", f"ac_sweep_temp_{version}", ac_sweep_temp))
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"ac_probes_only_{version}.ac0", f"ac_probes_only_{version}", ac_probes_only))
  ...
  def tran_sweep_temp(title, date, scale_name, sweep_name, sweep_vals, data_list):
    ...
    for i, t in enumerate(data_list):
      if sn not in t:
        errs.append(f"Table {i}: scale missing")
      elif len(t[sn]) < 2:
        errs.append(f"Table {i}: too few time points")
    return errs

  for version in ["9601", "2001", "2013"]:
    tests.append((f"tran_sweep_temp_{version}.tr0", f"tran_sweep_temp_{version}", tran_sweep_temp))
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"tran_sweep_param_{version}.tr0", f"tran_sweep_param_{version}", tran_sweep_param))
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"tran_sweep_source_{version}.tr0", f"tran_sweep_source_{version}", tran_sweep_source))

  # ------------------------------------------------------------------
  # TRAN PROBE AND SWEEP: rval 10..30 step 10 + probes v(node_a) v(node_b)
  # => 3 tables, each dict has 'node_a', 'node_b' keys
  # ------------------------------------------------------------------
  def tran_probe_and_sweep(title, date, scale_name, sweep_name, sweep_vals, data_list):
    errs = []
    if len(data_list) != 3:
      errs.append(f"Expected 3 tables, got {len(data_list)}")
      return errs
  ...
  for version in ["9601", "2001", "2013"]:
    tests.append((f"tran_probe_and_sweep_{version}.tr0", f"tran_probe_and_sweep_{version}", tran_probe_and_sweep))
...
def main():
  ...
  with open(log_path, "w") as lf:
    ...
    for filename, case_name, assertion_fn in tests:
      ...
      try:
        title, date, scale_name, sweep_name, sweep_vals, data_list = _unpack(result)
      ...
      log(f"    scale={scale_name!r}  sweep={sweep_name!r}"
          f"  sweep_vals={sweep_vals}  tables={len(data_list)}")
      if data_list:
        log(f"    table[0] keys: {list(data_list[0].keys())}")

      try:
        errs = assertion_fn(title, date, scale_name, sweep_name, sweep_vals, data_list)
      except Exception as e:
