...
def parse_header_payload(payload):
  ...
  if version_at_16 in ("9007", "9601"):
    ...
    raise ValueError(f"Unknown format version: pos16={repr(version_at_16)}, pos20={repr(version_at_20)}")
...
def parse_row(raw, offset, meta, var_sizes, is_complex, fmt):
  ...
  for idx in range(1, meta["num_vectors"]):
    ...
    if is_complex and idx > 0:
      ...
      circuit_vals.append(struct.unpack(fmt + vfmt, row[c_offset:c_offset + vsz])[0])
    c_offset += vsz
    ...
    if os.path.exists(meas_path):
      ...
      if meas is not None:
        ...
        # Swept measurements:
          ...
          if not isinstance(vals, list) or len(vals) != len(expected) or not all(is_close(x, y) for x, y in zip(vals, expected)):
...
def verify_file(filename, log):
  ...
  with open(filename, "rb") as f:
    ...
    head_raw = f.read(16)
    if len(head_raw) < 16:
      log("ERROR: truncated header block"); return False
    ...
    if e1 == 0x00000004 and e2 == 0x00000004:
      ...
      log(f"ERROR: bad endian markers 0x{e1:08x}/0x{e2:08x}"); return False

    log(f"Header Block Head: RecordCount={rc}, PayloadSize={payload_size}")
    payload = f.read(payload_size)
    if len(payload) < payload_size:
      log("ERROR: truncated header payload"); return False

    tail_raw = f.read(4)
    tail = struct.unpack(fmt + "I", tail_raw)[0]
    if tail != payload_size:
      log(f"ERROR: header head/tail mismatch ({payload_size} vs {tail})"); return False

    term_idx = payload.find(b"$&%#")
    if term_idx < 0:
      log("ERROR: '$&%#' termination marker not found in header payload"); return False
    log(f"Header '$&%#' terminator at offset {term_idx}")

    try:
      meta = parse_header_payload(payload)
    except Exception as e:
      log(f"ERROR parsing header: {e}"); return False
    ...
    while True:
      try:
        ...
        log(f"ERROR reading block {block_idx + 1}: {e}"); return False
    ...
    log(f"Read {block_idx} data blocks => {len(tables_raw)} sweep tables (expected {meta['sweep_size']})")

    if len(tables_raw) != meta["sweep_size"]:
      log(f"ERROR: got {len(tables_raw)} tables but expected {meta['sweep_size']}")
      return False
    ...
    for t_idx, t_raw in enumerate(tables_raw):
      ...
      if meta["num_sweeps"] > 0:
        if meta["version"] == "2001":
          if len(t_raw) < 8:
            log(f"    ERROR: too short for 8-byte sweep value"); return False
          sweep_val = struct.unpack(fmt + "d", t_raw[:8])[0]
          log(f"    Sweep value (double): {sweep_val:.4f}")
          offset += 8
        else:
          if len(t_raw) < 4:
            log(f"    ERROR: too short for 4-byte sweep value"); return False
      ...
      if data_len % row_size != 0:
        log(f"    ERROR: data_len={data_len} is not a multiple of row_size={row_size} (rem={data_len % row_size})")
        return False
      ...
      if num_rows > 1:
        ...
        sv_last, cv = parse_row(t_raw, last_off, meta, var_sizes, is_complex, fmt)
        log(f"    Last  row: {meta['var_names'][0]}={sv_last:.4e}  circuit={cv}")
...
def main():
  ...
  with open(log_path, "w") as lf:
    ...
    if not bin_files:
      log("No binary files found!"); sys.exit(1)
