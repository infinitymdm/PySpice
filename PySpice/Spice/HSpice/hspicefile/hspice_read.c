...
static int isModernFormatGlobal = 0;
static int swapGlobal = 0;
...
int readBlockHeader(FILE *f, const char *fileName, int debugMode,
                    int *blockHeader, int size) {
    ...
    if (blockHeader[0] == 0x00000004 && blockHeader[2] == 0x00000004) {
        ...
        if (debugMode) {
            fprintf(debugFile, "HSpiceRead: corrupted block header.\n");
        }
        return -1;
    }
    if (swap == 1) {
...
int readBlockTrailer(FILE *f, const char *fileName, int debugMode, int swap,
    ...
    if (header != trailer) {
        if (debugMode) {
            fprintf(debugFile,
                    "HSpiceRead: block header and trailer mismatch.\n");
        }
        return 1; // Error.
    }
    if (debugMode >= 2) {
...
int getSweepInfo(int debugMode, PyObject **sweep, char *buf, int *sweepSize,
    ...
    *sweep = PyUnicode_InternFromString(sweepName);
    if (*sweep == NULL) {
        if (debugMode) {
            fprintf(debugFile,
                    "HSpiceRead: failed to create sweep name string.\n");
        }
        return 1;
    }

    // Get number of sweep points.
    if (strncmp(&buf[postStartPosition2], postString21, numOfPostCharacters) !=
        0) {
        *sweepSize = atoi(&buf[sweepSizePosition1]);
    } else {
        *sweepSize = atoi(&buf[sweepSizePosition2]);
    }
...
int readDataBlock(FILE *f, int debugMode, const char *fileName, float **rawData,
    ...
    if (swap == -2) {
        return -1; // Normal EOF / End of block.
    }
    if (swap < 0) {
        return 1; // Error.
    }
    ...
    *rawData = tmpRawData;
    ...
    error = readBlockData(f, fileName, debugMode, *rawData + *rawDataOffset,
                          rawDataOffset, sizeof(float), blockHeader[0],
                          isModernFormatGlobal ? 0 : swap);
    if (error == 1) {
        return 1; // Error.
    }
...
int readTable(FILE *f, int debugMode, const char *fileName, PyObject *sweep,
              int numOfVariables, int type, int numOfVectors, int *varSizes,
    ...
    do {
        num = readDataBlock(f, debugMode, fileName, &rawData, &offset);
    } while (num == 0);
    if (num > 0) {
    ...
    if (data == NULL) {
        if (debugMode) {
            fprintf(debugFile,
                    "HSpiceRead: failed to create data dictionary.\n");
        }
        goto readTableFailed;
    }
    ...
    if (isModernFormatGlobal) {
        // Check for 1e30 double end marker
        if (tableBytes >= 8) {
            double lastVal;
    ...
    if (sweep != NULL) {
        if (isModernFormatGlobal) {
            double sweepVal;
            memcpy(&sweepVal, bytePtr, 8);
            if (swapGlobal) {
                do_swap((char *)&sweepVal, 1, 8);
            }
            *((npy_double *)(faSweep->pos)) = sweepVal;
            bytePtr += 8;
        } else {
            float sweepVal = *(float *)bytePtr;
            *((npy_double *)(faSweep->pos)) = sweepVal;
            bytePtr += 4;
        }
    ...
    // Increase number of columns if variables with exception of scale are
    // complex.
    if (type == complex_var) {
    ...
    for (i = 0; i < numOfVectors; i++) {
        ...
        if (type == complex_var && i > 0 && i < numOfVariables) {
            tmpArray[i] =
                (PyArrayObject *)PyArray_SimpleNew(1, &dims, NPY_CDOUBLE);
        } else {
            tmpArray[i] =
    ...
    if (isModernFormatGlobal) {
            ...
            for (j = 0; j < numOfVectors; j++) {
                ...
                if (type == complex_var && j > 0 && j < numOfVariables) {
                    ...
                    *((double *)(faPos->pos)) = val;
                    ...
                    float val;
                    memcpy(&val, bytePtr, 4);
                    if (swapGlobal) {
                        do_swap((char *)&val, 1, 4);
                    }
                    *((double *)(faPos->pos)) = val;
                }
                bytePtr += varSize;
        ...
        float *rawDataPos = (float *)bytePtr;
        for (i = 0; i < num; i++) {
            struct FastArray *faPos = faPtr;
            for (j = 0; j < numOfVectors; j++) {
                if (type == complex_var && j > 0 && j < numOfVariables) {
                    *(npy_cdouble *)(faPos->pos) =
                        npy_cpack(*rawDataPos, *(rawDataPos + 1));
                    rawDataPos = rawDataPos + 1;
                } else {
                    *((double *)(faPos->pos)) = *rawDataPos;
                }
                rawDataPos = rawDataPos + 1;
                faPos->pos = faPos->pos + faPos->stride;
                faPos = faPos + 1;
    ...
    if (num == 0) {
        for (i = 0; i < numOfVectors - 1; i++) {
            num = PyDict_SetItemString(data, name[i],
                                       (PyObject *)(tmpArray[i + 1]));
            if (num != 0) {
                break;
            }
        }
    ...
    if (num) {
        if (debugMode) {
            fprintf(debugFile, "HSpiceRead: failed to append table to the list "
...
static PyObject *HSpiceRead(PyObject *self, PyObject *args) {
    ...
    {
        if (debugMode) {
            fprintf(debugFile, "HSpiceRead: file %s is in ascii format.\n",
                    fileName);
        }
        goto failed;
    }
    ...
    do {
        num = readHeaderBlock(f, debugMode, fileName, &buf, &offset);
    } while (num == 0);
    if (num > 0) {
        goto failed;
    }
    ...
    if (offset < dateEndPosition + 1) {
        if (debugMode) {
            fprintf(debugFile, "HSpiceRead: file header is too short.\n");
        }
        goto failed;
    }
    ...
    if (strncmp(&buf[postStartPosition1], postString11, numOfPostCharacters) !=
            0 &&
        strncmp(&buf[postStartPosition1], postString12, numOfPostCharacters) !=
            0 &&
        strncmp(&buf[postStartPosition2], postString21, numOfPostCharacters) !=
            0) {
        int sub_len = 4;
        int idx;
        int found2013 = 0;
        for (idx = 0; idx <= offset - sub_len; idx++) {
            if (memcmp(buf + idx, "2013", sub_len) == 0) {
                isModernFormatGlobal = 1;
                found2013 = 1;
                break;
            }
        }
        if (!found2013) {
            if (debugMode) {
                fprintf(debugFile, "HSpiceRead: unknown post format.\n");
            }
            goto failed;
        }
    }
    ...
    date = PyUnicode_InternFromString(
        &buf[dateStartPosition]); // Get creation date.
    if (date == NULL) {
        if (debugMode) {
    ...
    buf[numOfSweepsPosition] =
        0; // Get number of vectors (variables and probes).
    ...
    if (varTypes == NULL) {
        if (debugMode) {
            fprintf(debugFile, "HSpiceRead: cannot allocate vector types.\n");
        }
        goto failed;
    }
    ...
    if (varSizes == NULL) {
        if (debugMode) {
            fprintf(debugFile, "HSpiceRead: cannot allocate vector sizes.\n");
        }
        goto failed;
    }
    ...
    if (token == NULL) {
        if (debugMode) {
            fprintf(debugFile,
                    "HSpiceRead: failed to extract vector description.\n");
        }
        goto failed;
    }
    if (numOfVectors > 1) {
    ...
    for (i = 0; i < numOfVectors; i++) {
        ...
        if (token == NULL) {
            if (debugMode) {
                fprintf(debugFile,
                        "HSpiceRead: failed to extract vector types/scale.\n");
            }
            goto failed;
        }
        if (numOfVectors > 1) {
    ...
    if (scale == NULL) {
        if (debugMode) {
            fprintf(debugFile, "HSpiceRead: failed to create independent "
    ...
    if (tmpArray == NULL) {
        if (debugMode) {
            fprintf(debugFile,
                    "HSpiceRead: cannot allocate pointers to arrays.\n");
        }
        goto failed;
    }

    // Allocate space for fast array pointers.
    faPtr = (struct FastArray *)PyMem_Malloc(numOfVectors *
                                             sizeof(struct FastArray));
    if (faPtr == NULL) {
        if (debugMode) {
            fprintf(debugFile, "HSpiceRead: failed to create array.\n");
        }
    ...
    for (i = 0; i < sweepSize; i++) // Read i-th table.
    {
        num = readTable(f, debugMode, fileName, sweep, numOfVariables, type,
                        numOfVectors, varSizes, &faSweep, tmpArray, faPtr,
                        token, name, dataList);
        if (num) {
            goto failed;
        }
